...
if __name__ == "__main__":
    ...
    if action == "add":
        ...
        # 5) Определяем домен (или поддомен) для VLESS-ссылки
        # Каждый пользователь получает поддомен <username>.<BASE_DOMAIN>
        subdomain = f"{username}.{BASE_DOMAIN}"

        # 6) Собираем VLESS-ссылку для клиента, используя поддомен вместо IP
        # Порт остаётся тем, который мы выделили из used_ports.txt
        vless_link = (
            f"vless://{uuid_str}@{subdomain}:{port}"
        ...
        print(vless_link)
        print()
        print(f"💡 Убедитесь, что запись DNS для {subdomain} указывает на IP ваших нод (или балансировщика).")
        ...
        cleanup_docker_system()
        print(f"💡 После миграции убедитесь, что DNS для {username}.{BASE_DOMAIN} всё ещё указывает на доступный узел.")
